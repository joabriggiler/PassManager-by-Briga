...
function mostrarOcultarPassword(elemento){
    // 1. EL CANDADO: Evita la doble ejecución simultánea
    if (elemento.dataset.toggling) return;
    elemento.dataset.toggling = "1";
    setTimeout(() => elemento.removeAttribute('data-toggling'), 100);

    // 2. Seleccionamos el SVG y el input de forma segura
    const svg = elemento.tagName.toLowerCase() === 'svg' ? elemento : elemento.closest('svg');
    ...
    // IMPORTANTE: Se lo inyectamos al svg, no a 'elemento'
    svg.innerHTML = input.type === 'password' ? pathVisible : pathHidden;

    if(input2) input2.type = input2.type === 'password' ? 'text' : 'password';

    clearTimeout(input._hideT);
    if (input.type === "text") {
        input._hideT = setTimeout(() => { 
            if (input.type === "text") { 
                input.type = "password"; 
                svg.innerHTML = pathVisible; 
